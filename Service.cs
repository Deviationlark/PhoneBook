...
internal static class Service
{
    internal static void ViewContacts()
    {
        ...
        {
            var contacts = Controller.Read();
            List<string> categories = contacts.Select(c => c.Category).ToList();
            List<string> categoryNames = new();
            foreach (var cat in categories)
            {
                if (categoryNames.Contains(cat)) continue;
                else categoryNames.Add(cat);
            }
    ...
    internal static Contact GetContactInput()
    {
        ...
        var contactList = contacts.Select(p => p.Name).ToList();
    ...
    internal static void SendEmail()
    {
        ...
        try
        {
            ...
            if (senderEmail == "0") UserInterface.MainMenu();
